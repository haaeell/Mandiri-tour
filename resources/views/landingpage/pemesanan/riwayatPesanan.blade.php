    ...
    <div class="container py-4">
        <div class="row d-flex">
            ...
            <div class="col-md-12 card" style="border-top-left-radius: 24px; border-top-right-radius: 24px;">
                <h2 class="fw-semibold text-center my-3 bg-primary p-3 text-light"
                    style="border-top-left-radius: 24px; border-top-right-radius: 24px;">Riwayat Pesanan</h2>
                                    ...
                                    <tr>
                                        ...
                                        <td style="cursor: pointer">
                                            {!! $item->bukti_pembayaran
                                                ? '<img src="' . asset('storage/' . $item->bukti_pembayaran) . '" width="50" onClick="showImage(this)">'
                                                : '<span class="fw-bold text-danger">Belum Dibayar</span>' !!}
                                        </td>
                                        <td>
                                            <div class="d-flex gap-2">
                                                @if (
                                                    !$item->bukti_pembayaran > 0 &&
                                                        $item->status_pembayaran != 'Pemesanan Dibatalkan' &&
                                                        $item->status_pembayaran != 'Pembayaran Diterima')
                                                    <form action="{{ route('pemesanan.cancel', $item->id) }}"
                                                        method="post">
                                                        @csrf
                                                        @method('post')
                                                ...
                                                @if ($item->status_pembayaran != 'Pemesanan Dibatalkan')
                                                    @if ($item->status_pembayaran != 'Pembayaran Diterima')
                                                        <a href="{{ route('pemesanan.invoice', $item->id) }}"
