...
<body>
    <nav class="navbar navbar-expand-lg  bg-body-tertiary shadow-md"
        style="background: rgb(25,185,227);
        ...
        <div class="container">
            ...
            <div class="navbar-nav ">
                ...
                <a class="nav-link active  text-white fw-bold" aria-current="page"
                    href="https://api.whatsapp.com/send?phone=6285321726312&text=Halo,%20saya%20ingin%20mendapatkan%20informasi%20mengenai%20Mandiri%20Tour%20&%20Travel"
                    target="_blank"><i class="bi bi-telephone-forward-fill"></i> Contact Us</a>
    ...
    <div class=" {{ request()->is('/') ? 'sticky-top' : '' }}">
        ...
        <nav class="navbar navbar-expand-lg navbar-light nav-parent  border-bottom border-2"
            style="background-color: white">
            <div class="container">
                <img src="../assets/img/logo2.png" class="pt-3" style="max-width: 100px; height: auto;"
                    alt="">
                ...
                <div class="collapse navbar-collapse" id="navbarNav">
                    <ul class="navbar-nav mx-auto gap-4 fw-semibold  ">
                        <li class="nav-item">
                            <a class="nav-link nav-menu {{ request()->is('/') ? 'menu-active' : '' }}"
                                href="{{ route('welcome') }}">Home</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link nav-menu {{ request()->is('landingpage/wisata') ? 'menu-active' : '' }}"
                                href="{{ route('landingpage.wisata') }}">Wisata</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link nav-menu {{ request()->is('paket') ? 'menu-active' : '' }}"
                                href="{{ route('paketWisata') }}">Paket Wisata</a>
                        </li>
                        @if (Auth::check())

                            <li class="nav-item">
                                <a class="nav-link nav-menu {{ request()->is('keluhan') ? 'menu-active' : '' }}"
                                    href="{{ route('keluhan') }}">
                                    Keluhan
                                    @php
                                        $unreadKeluhanCount = Auth::user()
                                            ->unreadNotifications->where(
                                                'type',
                                                'App\Notifications\KeluhanDitanggapiNotification',
                                            )
                                            ->count();
                                    @endphp
                                    ...
                                    @if ($unreadKeluhanCount > 0)
                                        <span
                                            class="position-absolute top-20 start-70 translate-middle badge rounded-pill bg-danger">
                                            {{ $unreadKeluhanCount }}
                                        </span>
                                    @endif
                                </a>
                            ...
                            <li class="nav-item">
                                <a class="nav-link nav-menu {{ request()->is('riwayat-pesanan') ? 'menu-active' : '' }}"
                                    href="{{ route('riwayatPesanan') }}">Riwayat Pesanan
                                    @php
                                        $unreadKeluhanCount = Auth::user()
                                            ->unreadNotifications->where(
                                                'type',
                                                'App\Notifications\KonfirmasiPembayaran',
                                            )
                                            ->count();
                                    @endphp
                    ...
                    <ul class="navbar-nav">
                        @if (Route::has('login'))
                                    ...
                                    <li class="nav-item dropdown">
                                        <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown"
                                            ...
                                            <img src="{{ Auth::user()->image ? asset('/images/' . Auth::user()->image) : asset('assets/img/profile.png') }}"
                                                ...
                                                width="50"
                                                style="margin-right: 10px; border-radius: 50%;width: 40px; height: 40px">
                                            {{ Auth::user()->name }}
                                        </a>
                            ...
                            @else
                                <li class="nav-item">
                                    <a class="fw-bold nav-link " href="{{ route('login') }}"><button
                                            class="btn-login bn26">Login</button></a>
                                </li>
                                @if (Route::has('register'))
                                    <li class="nav-item">
                                        <a class="fw-bold nav-link nav-menu" href="{{ route('register') }}"><button
...
<a href="https://api.whatsapp.com/send?phone=6285321726312&text=Halo,%20saya%20ingin%20mendapatkan%20informasi%20mengenai%20Mandiri%20Tour%20&%20Travel"
    target="_blank" class="whatsapp-btn"><i class="bi bi-whatsapp"></i></a>
